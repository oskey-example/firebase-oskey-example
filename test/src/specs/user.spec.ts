...
function checkUserDocumentUpdated(userDoc: firebaseFirestore.DocumentSnapshot<any>, user: OSKTDUser, documentPath: string) {
  ...
  expect(userData.creationDate, 'Creation date is not set').to.exist;
...
function checkUserStatusDocumentUpdated(userStatusDoc: firebaseFirestore.DocumentSnapshot<any>, documentPath: string) {
  ...
  expect(userStatusData.creationDate, 'Creation date is not set').to.exist;
};

export function testUser(apps: OSKUnitTestApps) {
  ...
  for (const user of users) {
    describe(`User ${user.auth.email}`, async () => {
      ...
      // For user with profile data, complete the user profile
      if (user.data) {
